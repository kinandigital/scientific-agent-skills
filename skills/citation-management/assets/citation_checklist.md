...
- [ ] Run: `uv run python scripts/validate_citations.py references.bib --check-dois`
...
- [ ] Run: `uv run python scripts/validate_citations.py references.bib`
...
- [ ] Run: `uv run python scripts/format_bibtex.py references.bib --sort year`
...
```bash
uv run python scripts/format_bibtex.py references.bib \
  --deduplicate \
  --sort year \
  --descending \
...
```bash
uv run python scripts/validate_citations.py clean_references.bib \
  --check-dois \
...
```bash
uv run python scripts/validate_citations.py clean_references.bib --verbose
```
...
# Re-extract metadata
uv run python scripts/extract_metadata.py --doi CORRECT_DOI
```
...
```bash
# Extract from DOI
uv run python scripts/extract_metadata.py --doi 10.xxxx/yyyy

# Or from PMID (biomedical)
uv run python scripts/extract_metadata.py --pmid 12345678

# Or from arXiv
uv run python scripts/extract_metadata.py --arxiv 2103.12345
```
...
# Then validate
uv run python scripts/validate_citations.py fixed_references.bib
```
...
```bash
# From DOI
uv run python scripts/doi_to_bibtex.py 10.1038/nature12345
...
```bash
# Basic validation
uv run python scripts/validate_citations.py references.bib

# With DOI checking (slow but thorough)
uv run python scripts/validate_citations.py references.bib --check-dois
