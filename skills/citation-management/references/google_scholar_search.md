...
**In script**:
```bash
uv run python scripts/search_google_scholar.py "quantum computing" \
  --year-start 2020 \
  --year-end 2024
```
...
```bash
uv run python scripts/search_google_scholar.py "transformers" \
  --sort-by citations \
  --limit 50
```
...
**Example**:
```bash
uv run python scripts/search_google_scholar.py "AlphaFold protein structure" \
  --year-start 2023 \
  --year-end 2024 \
  --limit 50
```
...
5. **Export results** for systematic analysis:
   ```bash
   uv run python scripts/search_google_scholar.py \
...
4. **Export systematically**:
   ...
   uv run python scripts/search_google_scholar.py "your topic" \
...
**In script**:
```bash
uv run python scripts/search_google_scholar.py "topic" \
  --open-access-only \
...
3. Use deduplication in post-processing:
   ```bash
   uv run python scripts/format_bibtex.py results.bib \
     --deduplicate \
...
**Basic search**:
```bash
uv run python scripts/search_google_scholar.py "machine learning drug discovery"
```

**With year filter**:
```bash
uv run python scripts/search_google_scholar.py "CRISPR" \
  --year-start 2020 \
  --year-end 2024 \
  --limit 100
```
...
# Search each query
while read query; do
  uv run python scripts/search_google_scholar.py "$query" \
    --limit 50 \
    --output "${query// /_}.json"
