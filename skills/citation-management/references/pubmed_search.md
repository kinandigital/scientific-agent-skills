...
**Basic search**:
```bash
uv run python scripts/search_pubmed.py "diabetes treatment"
```

**With MeSH terms**:
```bash
uv run python scripts/search_pubmed.py \
  --query '"Diabetes Mellitus"[MeSH] AND "Drug Therapy"[MeSH]'
```

**Date range filter**:
```bash
uv run python scripts/search_pubmed.py "CRISPR" \
...
```bash
uv run python scripts/search_pubmed.py "cancer immunotherapy" \
...
**Export to BibTeX**:
```bash
uv run python scripts/search_pubmed.py "Alzheimer's disease" \
  --limit 100 \
  --format bibtex \
...
# Run search
uv run python scripts/search_pubmed.py --query-file query.txt --limit 500
```
...
for topic in "${TOPICS[@]}"; do
  uv run python scripts/search_pubmed.py "$topic" \
    --limit 100 \
    --output "${topic// /_}.json"
  sleep 1
done
...
uv run python scripts/search_pubmed.py "topic" --output results.json
...
3. **Validate retrieved citations**:
   ```bash
   uv run python scripts/validate_citations.py pubmed_results.bib
   ```
