...
```bash
uv run python scripts/validate_citations.py references.bib --check-required-fields
```
...
```bash
uv run python scripts/validate_citations.py references.bib --check-authors
```
...
**Automated detection**:
```bash
uv run python scripts/validate_citations.py references.bib --check-duplicates
```
...
```bash
uv run python scripts/validate_citations.py references.bib --check-syntax
```
...
```bash
uv run python scripts/validate_citations.py fixed_references.bib --verbose
```
...
```bash
# After extraction
uv run python scripts/extract_metadata.py --doi ... --output refs.bib
uv run python scripts/validate_citations.py refs.bib

# After manual edits
uv run python scripts/validate_citations.py refs.bib

# Before submission
uv run python scripts/validate_citations.py refs.bib --strict
```
