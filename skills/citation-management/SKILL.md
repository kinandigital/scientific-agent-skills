...
uv run python scripts/search_google_scholar.py "CRISPR gene editing" \
  --limit 50 \
  --output results.json

# Search with year filter
uv run python scripts/search_google_scholar.py "machine learning protein folding" \
  --year-start 2020 \
  --year-end 2024 \
  --limit 100 \
...
```bash
# Search PubMed
uv run python scripts/search_pubmed.py "Alzheimer's disease treatment" \
  --limit 100 \
...
```bash
# Convert single DOI
uv run python scripts/doi_to_bibtex.py 10.1038/s41586-021-03819-2

# Convert multiple DOIs from a file
uv run python scripts/doi_to_bibtex.py --input dois.txt --output references.bib

# Different output formats
uv run python scripts/doi_to_bibtex.py 10.1038/nature12345 --format json
```
...
# Extract from URL
uv run python scripts/extract_metadata.py --url "https://www.nature.com/articles/s41586-021-03819-2"

# Batch extraction from file (mixed identifiers)
uv run python scripts/extract_metadata.py --input identifiers.txt --output citations.bib
```
...
uv run python scripts/validate_citations.py references.bib --venue nature
uv run python scripts/validate_citations.py references.bib --venue neurips
uv run python scripts/validate_citations.py references.bib --venue review

# Validate with custom minimum citation count
uv run python scripts/validate_citations.py references.bib --min-count 40

# Check references against a written manuscript file (detect missing or unused citations)
uv run python scripts/validate_citations.py references.bib --manuscript paper.md
...
uv run python scripts/doi_to_bibtex.py 10.1038/nature12345 >> crispr_refs.bib
uv run python scripts/doi_to_bibtex.py 10.1126/science.abcd1234 >> crispr_refs.bib

# 4. Format and clean the BibTeX file
uv run python scripts/format_bibtex.py crispr_refs.bib \
  --deduplicate \
  --sort year \
  --descending \
...
uv run python scripts/validate_citations.py my_review_references.bib --report review_validation.json

# Format for specific citation style if needed
uv run python scripts/format_bibtex.py my_review_references.bib \
  --style nature \
...
uv run python scripts/search_google_scholar.py "machine learning" \
  --limit 50 \
  --format bibtex \
...
uv run python scripts/search_pubmed.py "CRISPR gene editing"
...
# Multiple DOIs
uv run python scripts/doi_to_bibtex.py \
  10.1038/nature12345 \
...
# Copy to clipboard
uv run python scripts/doi_to_bibtex.py 10.1038/nature12345 --clipboard
```
...
uv run python scripts/search_google_scholar.py "transformer neural networks" \
  --year-start 2017 \
  --limit 50 \
  --output transformers_gs.json

uv run python scripts/search_pubmed.py "deep learning medical imaging" \
  --date-start 2020 \
  --limit 50 \
...
uv run python scripts/doi_to_bibtex.py 10.1038/s41586-021-03819-2 >> specific.bib
uv run python scripts/doi_to_bibtex.py 10.1126/science.aam9317 >> specific.bib

# Step 4: Combine all BibTeX files
cat transformers.bib medical.bib specific.bib > combined.bib

# Step 5: Format and deduplicate
uv run python scripts/format_bibtex.py combined.bib \
  --deduplicate \
  --sort year \
  --descending \
...
# Step 6: Validate
uv run python scripts/validate_citations.py formatted.bib \
  --auto-fix \
  --report validation.json \
  --output final_references.bib
...
# Validate the result
uv run python scripts/validate_citations.py references.bib --verbose
```
...
uv run python scripts/format_bibtex.py messy_references.bib \
  --output step1_formatted.bib

# Step 2: Remove duplicates
uv run python scripts/format_bibtex.py step1_formatted.bib \
  --deduplicate \
  --output step2_deduplicated.bib

# Step 3: Validate and auto-fix
uv run python scripts/validate_citations.py step2_deduplicated.bib \
  --auto-fix \
  --output step3_validated.bib

# Step 4: Sort by year
uv run python scripts/format_bibtex.py step3_validated.bib \
  --sort year \
  --descending \
  --output clean_references.bib
