...
```bash
uv run python scripts/tree.py init \
  --objective "Improve BrowseComp answer accuracy on the search harness" \
  --dev-eval "uv run python eval.py --split dev --n 50" \
  --test-eval "uv run python eval.py --split test --n 300" \
...
You run repeated cycles of six steps. This is the heart of HTR; do not collapse it into ad-hoc editing. Run `uv run python scripts/tree.py cycle` once per cycle to track the budget.

### 1. Observe
Begin every cycle by re-grounding in the tree, not in your memory of the conversation:

```bash
uv run python scripts/tree.py observe
```
...
```bash
uv run python scripts/tree.py add-node --parent n0 --hypothesis "Verification, not retrieval, is the bottleneck: candidates are found but discarded"
uv run python scripts/tree.py add-node --parent n4 --hypothesis "Decompose the question into atomic constraints and verify each independently"
```
...
```bash
uv run python scripts/tree.py set-evidence --node n5 --dev-score 70.0 \
...
uv run python scripts/tree.py propagate --node n5 \
...
- **Prune** dead ends, recording *why* — the reason becomes a negative constraint:
  ```bash
  uv run python scripts/tree.py prune --node n7 --reason "search-augmented judge overfits dev questions; no test transfer"
  ```
- **Merge gate** — promote a candidate to the new best **only if it improves on `E_test`**. Run the test evaluator in a *fresh* worktree (not the dev worktree, to avoid leakage), then:
  ```bash
  uv run python scripts/tree.py merge --node n5 --test-score 67.67 --branch-ref "wt/n5"
  ```
...
- the tree (`uv run python scripts/tree.py status`) as the audit trail of what was tried;
