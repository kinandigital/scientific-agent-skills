...
```bash
uv run python scripts/validate_files.py --bam sample1.bam sample2.bam --bed regions.bed
```
...
uv run python scripts/workflow_generator.py --list

# Generate ChIP-seq QC workflow
uv run python scripts/workflow_generator.py chipseq_qc -o qc_workflow.sh \
...
```bash
uv run python scripts/validate_files.py --bam *.bam --bed regions.bed
```
...
**Usage:**
```bash
uv run python scripts/validate_files.py --bam sample1.bam sample2.bam \
...
# Generate workflow
uv run python scripts/workflow_generator.py chipseq_qc -o qc.sh \
    --input-bam Input.bam --chip-bams "ChIP1.bam ChIP2.bam" \
