...
Examples:
  ...
  uv run python research_lookup.py "latest advances in quantum computing 2025"

  # Academic paper search (auto-routes to Perplexity)
  uv run python research_lookup.py "find papers on CRISPR gene editing clinical trials"

  # Force a specific backend
  uv run python research_lookup.py "topic" --force-backend parallel
  uv run python research_lookup.py "topic" --force-backend perplexity

  # Save output to file
  uv run python research_lookup.py "topic" -o results.txt

  # JSON output
  uv run python research_lookup.py "topic" --json -o results.json
        """,
    )
