...
3. **Test Setup:**
   ```bash
   uv run python scripts/research_lookup.py --model-info
   ```
...
uv run python scripts/research_lookup.py "Recent advances in CRISPR gene editing 2024"

# Multiple queries with delay
uv run python scripts/research_lookup.py --batch "CRISPR applications" "gene therapy trials" "ethical considerations"

# Claude Code integration (called automatically)
uv run python lookup.py "your research query here"
```
