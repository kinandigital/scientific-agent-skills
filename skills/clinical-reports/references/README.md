...
uv run python generate_report_template.py

# Or specify type directly
uv run python generate_report_template.py --type case_report --output my_case_report.md
```

### Validate a Case Report

```bash
uv run python validate_case_report.py my_case_report.md
```

### Check De-identification

```bash
uv run python check_deidentification.py my_case_report.md
```

### Validate Clinical Trial Report

```bash
uv run python validate_trial_report.py my_csr.md
```
...
2. **Generate template**: `uv run python generate_report_template.py --type case_report`
3. **Write case report** following CARE structure
4. **Validate compliance**: `uv run python validate_case_report.py case_report.md`
5. **Check de-identification**: `uv run python check_deidentification.py case_report.md`
6. **Submit to journal** with CARE checklist

### Clinical Trial SAE Workflow

1. **Generate SAE template**: `uv run python generate_report_template.py --type sae`
2. **Complete SAE form** within 24 hours of event
3. **Assess causality** using WHO-UMC or Naranjo criteria
4. **Validate completeness**: `uv run python validate_trial_report.py sae_report.md`
