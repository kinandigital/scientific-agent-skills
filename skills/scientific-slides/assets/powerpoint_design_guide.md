...
uv run python scripts/thumbnail.py presentation.pptx review/thumbnails --cols 4

# Or for individual slides
uv run python scripts/thumbnail.py presentation.pptx review/slide
```
...
```bash
uv run python scripts/inventory.py template.pptx inventory.json
```

2. **Create thumbnail grid**:
```bash
uv run python scripts/thumbnail.py template.pptx template_review
```
...
4. **Rearrange slides**:
```bash
uv run python scripts/rearrange.py template.pptx working.pptx 0,5,5,12,18,22
```

5. **Replace content**:
```bash
uv run python scripts/replace.py working.pptx replacements.json output.pptx
```
