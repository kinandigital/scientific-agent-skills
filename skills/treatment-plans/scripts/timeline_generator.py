...
def create_visual_timeline(timeline_data: Dict, output_file: Path, start_date: str = None):
    ...
    if not HAS_MATPLOTLIB:
        print("Error: matplotlib not installed. Install with: uv pip install matplotlib", file=sys.stderr)
...
Examples:
  ...
  uv run python timeline_generator.py --plan my_plan.tex

  # Generate visual timeline (requires matplotlib)
  uv run python timeline_generator.py --plan my_plan.tex --output timeline.png --visual

  # Specify start date for visual timeline
  uv run python timeline_generator.py --plan my_plan.tex --output timeline.pdf --visual --start 2025-02-01
