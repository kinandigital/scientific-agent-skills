...
```bash
uv run python check_completeness.py my_treatment_plan.tex
```
...
```bash
uv run python validate_treatment_plan.py my_treatment_plan.tex
```
...
```bash
uv run python timeline_generator.py --plan my_treatment_plan.tex --output timeline.pdf
```
...
uv run python generate_template.py

# Or specify type directly
uv run python generate_template.py --type mental_health --output depression_treatment_plan.tex
```

### Validation Workflow

1. **Create treatment plan** using appropriate LaTeX template
2. **Check completeness**: `uv run python check_completeness.py plan.tex`
3. **Validate quality**: `uv run python validate_treatment_plan.py plan.tex`
