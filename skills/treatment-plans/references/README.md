...
uv run python generate_template.py --type general_medical --output diabetes_plan.tex
```
...
3. **Generate template** - `uv run python generate_template.py --type [type]`
...
9. **Validate completeness** - `uv run python check_completeness.py plan.tex`
10. **Quality check** - `uv run python validate_treatment_plan.py plan.tex`
