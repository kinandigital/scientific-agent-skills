...
5. **Recalculate formulas (MANDATORY IF USING FORMULAS)**: Use the `scripts/recalc.py` script
   ```bash
   uv run python skills/xlsx/scripts/recalc.py output.xlsx
   ```
...
```bash
uv run python skills/xlsx/scripts/recalc.py <excel_file> [timeout_seconds]
```

Example:
```bash
uv run python skills/xlsx/scripts/recalc.py output.xlsx 30
```
