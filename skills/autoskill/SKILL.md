...
```bash
uv run python scripts/autoskill.py doctor   --config config.yaml --skills-dir ../
uv run python scripts/autoskill.py run      --start ... --end ... --config config.yaml
uv run python scripts/autoskill.py promote  --proposed ~/.autoskill/proposed/<ts> --skills-dir ../ --name <skill>
```
...
```bash
uv run python scripts/autoskill.py doctor \
...
```bash
export SCREENPIPE_TOKEN=$(screenpipe auth token)
uv run python scripts/autoskill.py run \
...
```bash
uv run python scripts/autoskill.py promote \
