...
Usage:
    uv run python process_identifications.py search.idXML --out filtered.idXML --csv hits.csv
    uv run python process_identifications.py search.idXML --fasta db.fasta --fdr 0.01 --out filt.idXML
    uv run python process_identifications.py search.idXML --fdr 0.05 --min-length 7 --best-per-spectrum
"""
