...
Run with `uv run python scripts/<name>.py --help` for full options. All accept standard
...
```bash
# Inspect a file
uv run python scripts/inspect_ms_data.py sample.mzML --spectra-csv spectra.csv

# Untargeted metabolomics: features for one sample
uv run python scripts/detect_features_metabo.py sample.mzML --out-csv features.csv

# Full multi-sample quantification study
uv run python scripts/align_link_quantify.py s1.mzML s2.mzML s3.mzML --out-prefix study
uv run python scripts/consensus_to_matrix.py study.consensusXML --out quant.csv --normalize median

# Peptide chemistry
uv run python scripts/mass_calculator.py --peptide "PEPTIDEM(Oxidation)K" --charges 1 2 3 --isotopes 5
uv run python scripts/digest_protein.py proteins.fasta --enzyme Trypsin --missed 2 --out peptides.csv

# Identification post-processing
uv run python scripts/process_identifications.py search.idXML --fasta db.fasta --fdr 0.01 --out filtered.idXML --csv hits.csv
```
