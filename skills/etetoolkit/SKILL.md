...
uv run python scripts/tree_operations.py stats tree.nw

# Convert format
uv run python scripts/tree_operations.py convert tree.nw output.nw --in-format 0 --out-format 1

# Reroot tree
uv run python scripts/tree_operations.py reroot tree.nw rooted.nw --midpoint

# Prune to specific taxa
uv run python scripts/tree_operations.py prune tree.nw pruned.nw --keep-taxa "sp1,sp2,sp3"

# Show ASCII visualization
uv run python scripts/tree_operations.py ascii tree.nw
```
...
```bash
# Basic visualization
uv run python scripts/quick_visualize.py tree.nw output.pdf

# Circular layout with custom styling
uv run python scripts/quick_visualize.py tree.nw output.pdf --mode c --color-by-support

# High-resolution PNG
uv run python scripts/quick_visualize.py tree.nw output.png --width 1200 --height 800 --units px --dpi 300

# Custom title and styling
uv run python scripts/quick_visualize.py tree.nw output.pdf --title "Species Phylogeny" --show-support
```
...
```bash
# Convert format
uv run python scripts/tree_operations.py convert input.nw output.nw --in-format 0 --out-format 1

# Root at midpoint
uv run python scripts/tree_operations.py reroot input.nw rooted.nw --midpoint

# Prune to focal taxa
uv run python scripts/tree_operations.py prune rooted.nw pruned.nw --keep-taxa taxa_list.txt

# Get statistics
uv run python scripts/tree_operations.py stats pruned.nw
```
