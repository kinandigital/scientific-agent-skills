...
uv run python scripts/protein_analysis_workflow.py ZAP70_HUMAN
# Or pass email as optional second argument if NCBI_EMAIL is unset
uv run python scripts/protein_analysis_workflow.py ZAP70_HUMAN your.email@example.com
```
...
```bash
uv run python scripts/pathway_analysis.py hsa output_directory/
```
...
```bash
uv run python scripts/compound_cross_reference.py Geldanamycin
```
...
```bash
uv run python scripts/batch_id_converter.py input_ids.txt --from UniProtKB_AC-ID --to KEGG
```
