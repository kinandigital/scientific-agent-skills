...
Usage:
    uv run python batch_id_converter.py INPUT_FILE --from DB1 --to DB2 [options]

Examples:
    uv run python batch_id_converter.py uniprot_ids.txt --from UniProtKB_AC-ID --to KEGG
    uv run python batch_id_converter.py gene_ids.txt --from GeneID --to UniProtKB --output mapping.csv
    uv run python batch_id_converter.py ids.txt --from UniProtKB_AC-ID --to Ensembl --chunk-size 50
...
Examples:
  uv run python batch_id_converter.py uniprot_ids.txt --from UniProtKB_AC-ID --to KEGG
  uv run python batch_id_converter.py ids.txt --from GeneID --to UniProtKB -o mapping.csv
  uv run python batch_id_converter.py ids.txt --from uniprot --to ensembl --chunk-size 50
