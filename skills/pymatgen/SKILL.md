...
uv run python scripts/structure_converter.py POSCAR structure.cif

# Batch conversion
uv run python scripts/structure_converter.py *.cif --output-dir ./poscar_files --format poscar
```
...
uv run python scripts/structure_analyzer.py POSCAR --symmetry --neighbors

# Export results
uv run python scripts/structure_analyzer.py structure.cif --symmetry --export json
```
...
uv run python scripts/phase_diagram_generator.py Li-Fe-O --output li_fe_o.png

# Analyze specific composition
uv run python scripts/phase_diagram_generator.py Li-Fe-O --analyze "LiFeO2" --show
```
...
- **`structure_converter.py`**: Convert between structure file formats
  ...
  - Usage: `uv run python scripts/structure_converter.py POSCAR structure.cif`

- **`structure_analyzer.py`**: Comprehensive structure analysis
  - Symmetry, coordination, lattice parameters, distance matrix
  - Usage: `uv run python scripts/structure_analyzer.py structure.cif --symmetry --neighbors`

- **`phase_diagram_generator.py`**: Generate phase diagrams from Materials Project
  - Stability analysis and thermodynamic properties
  - Usage: `uv run python scripts/phase_diagram_generator.py Li-Fe-O --analyze "LiFeO2"`

All scripts include detailed help: `uv run python scripts/script_name.py --help`
