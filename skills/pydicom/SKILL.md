...
Use the script: `uv run python scripts/dicom_to_image.py input.dcm output.png`
...
Use the provided script: `uv run python scripts/anonymize_dicom.py input.dcm output.dcm`
...
```bash
uv run python scripts/anonymize_dicom.py input.dcm output.dcm
```
...
```bash
uv run python scripts/dicom_to_image.py input.dcm output.png
uv run python scripts/dicom_to_image.py input.dcm output.jpg --format JPEG
```
...
```bash
uv run python scripts/extract_metadata.py file.dcm
uv run python scripts/extract_metadata.py file.dcm --output metadata.txt
```
