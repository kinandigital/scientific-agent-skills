...
Usage:
    uv run python dicom_to_image.py input.dcm output.png
    uv run python dicom_to_image.py input.dcm output.jpg --format JPEG
    uv run python dicom_to_image.py input.dcm output.tiff --apply-windowing
"""
...
try:
    ...
    print("Install with: uv pip install pydicom pillow numpy")
    sys.exit(1)
...
Examples:
  ...
  uv run python dicom_to_image.py input.dcm output.jpg --format JPEG
  uv run python dicom_to_image.py input.dcm output.tiff --apply-windowing
  uv run python dicom_to_image.py multiframe.dcm frame5.png --frame 5
        """
    )
