...
Usage:
    uv run python extract_metadata.py file.dcm
    uv run python extract_metadata.py file.dcm --output metadata.txt
    uv run python extract_metadata.py file.dcm --format json --output metadata.json
"""
...
try:
    ...
    print("Error: pydicom is not installed. Install it with: uv pip install pydicom")
    sys.exit(1)
...
Examples:
  ...
  uv run python extract_metadata.py file.dcm --output metadata.txt
  uv run python extract_metadata.py file.dcm --format json --output metadata.json
  uv run python extract_metadata.py file.dcm --show-sequences
        """
    )
