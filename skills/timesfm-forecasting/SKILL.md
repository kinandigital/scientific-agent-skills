...
```bash
uv run python scripts/forecast_csv.py input.csv \
    --horizon 24 \
    --date-col date \
...
cd examples/global-temperature && uv run python run_forecast.py && uv run python visualize_forecast.py

# Anomaly detection (uses TimesFM 1.0)
cd examples/anomaly-detection && uv run python detect_anomalies.py

# Covariates (API demo -- requires TimesFM 2.5 + timesfm[xreg] for real inference)
cd examples/covariates-forecasting && uv run python demo_covariates.py
```
