...
Usage:
    uv run python forecast_csv.py input.csv --horizon 24
    uv run python forecast_csv.py input.csv --horizon 12 --date-col date --value-cols sales,revenue
    uv run python forecast_csv.py input.csv --horizon 52 --output forecasts.csv
    uv run python forecast_csv.py input.csv --horizon 30 --output forecasts.json --format json
...
def run_preflight() -> dict:
    ...
    if not report.passed:
        ...
        print("\nRun 'uv run python scripts/check_system.py' for details.")
        sys.exit(1)
