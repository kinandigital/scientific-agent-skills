...
Always use `uv add` (never `uv pip install` or `conda install`) in all install instructions, docstrings, comments, and error messages.
...
```bash
# Command line
uv run python -m cuml.accel script.py
uv run python -m cuml.accel -v script.py     # With info logging
uv run python -m cuml.accel -vv script.py    # With debug logging
```
...
CUML_ACCEL_ENABLED=1 uv run python script.py
```
