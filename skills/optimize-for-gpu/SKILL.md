...
IMPORTANT: Always use `uv add` for package installation — never `uv pip install` or `conda install`. This applies to install instructions in code comments, docstrings, error messages, and any other output you generate. If the user's project uses a different package manager, follow their lead, but default to `uv add`.
...
# Load it with: uv run python -m cudf.pandas your_script.py
...
# Load it with: uv run python -m cuml.accel your_script.py
...
# NX_CUGRAPH_AUTOCONFIG=True uv run python your_script.py
...
# Or zero-code-change: uv run python -m cudf.pandas your_script.py
```
...
# Or zero-code-change: NX_CUGRAPH_AUTOCONFIG=True uv run python your_script.py
```
...
# Or zero-code-change: uv run python -m cuml.accel your_script.py
```
