...
3. **Export and Aggregate Results**:
   ...
   - Use `scripts/search_databases.py` for post-processing:
     ```bash
     uv run python search_databases.py combined_results.json \
       --deduplicate \
       --format markdown \
...
1. **Deduplication**:
   ```bash
   uv run python search_databases.py results.json --deduplicate --output unique_results.json
   ```
...
1. **Verify All DOIs**:
   ```bash
   uv run python scripts/verify_citations.py my_literature_review.md
   ```
...
1. **Generate PDF**:
   ```bash
   uv run python scripts/generate_pdf.py my_literature_review.md \
     --citation-style apa \
...
uv run python scripts/search_databases.py combined_results.json \
  --deduplicate \
  --rank citations \
  --year-start 2015 \
  --year-end 2024 \
...
uv run python scripts/verify_citations.py crispr_sickle_cell_review.md
...
uv run python scripts/generate_pdf.py crispr_sickle_cell_review.md \
...
Check dependencies:
```bash
uv run python scripts/generate_pdf.py --check-deps
```
