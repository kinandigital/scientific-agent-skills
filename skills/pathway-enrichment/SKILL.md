...
uv run python scripts/run_enrichment.py ora \
...
uv run python scripts/run_enrichment.py gsea \
...
uv run python scripts/run_enrichment.py gsea --rnk ranked_genes.csv --outdir results/
```

Run `uv run python scripts/run_enrichment.py --help` for all options (background file, FDR cutoff, min/max set size, permutations).
