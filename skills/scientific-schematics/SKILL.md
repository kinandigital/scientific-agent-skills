...
uv run python scripts/generate_schematic.py "CONSORT participant flow diagram with 500 screened, 150 excluded, 350 randomized" -o figures/consort.png --doc-type journal

# Generate for presentation (lower threshold: 6.5/10 - faster)
uv run python scripts/generate_schematic.py "Transformer encoder-decoder architecture showing multi-head attention" -o figures/transformer.png --doc-type presentation

# Generate for poster (moderate threshold: 7.0/10)
uv run python scripts/generate_schematic.py "MAPK signaling pathway from EGFR to gene transcription" -o figures/mapk_pathway.png --doc-type poster

# Custom max iterations (max 2)
uv run python scripts/generate_schematic.py "Complex circuit diagram with op-amp, resistors, and capacitors" -o figures/circuit.png --iterations 2 --doc-type journal
```
...
```bash
uv run python scripts/generate_schematic.py "your diagram description" -o output.png
```
...
uv run python scripts/generate_schematic.py "diagram description" -o output.png

# Specify document type for appropriate quality threshold
uv run python scripts/generate_schematic.py "diagram" -o out.png --doc-type journal      # 8.5/10
uv run python scripts/generate_schematic.py "diagram" -o out.png --doc-type conference   # 8.0/10
uv run python scripts/generate_schematic.py "diagram" -o out.png --doc-type poster       # 7.0/10
uv run python scripts/generate_schematic.py "diagram" -o out.png --doc-type presentation # 6.5/10

# Custom max iterations (1-2)
uv run python scripts/generate_schematic.py "complex diagram" -o diagram.png --iterations 2

# Verbose output (see all API calls and reviews)
uv run python scripts/generate_schematic.py "flowchart" -o flow.png -v

# Provide API key via flag
uv run python scripts/generate_schematic.py "diagram" -o out.png --api-key "sk-or-v1-..."

# Combine options
uv run python scripts/generate_schematic.py "neural network" -o nn.png --doc-type journal --iterations 2 -v
```
...
```bash
uv run python scripts/generate_schematic.py \
...
# Verbose mode
uv run python scripts/generate_schematic.py "diagram" -o out.png -v
```
